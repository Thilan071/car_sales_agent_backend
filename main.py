...
# Define tools and workflow
tools = [get_car_details, get_negotiation_strategy, calculate_payment_options]
tool_node = ToolNode(tools)
model_with_tools = llm.bind_tools(tools=tools)
...
def call_model(state: MessagesState):
    messages = state["messages"]
    prompt_template = """
    You are a professional car sales representative for ABC company. Help the customer by:
    1. Understanding their needs
    2. Providing car details
    3. Assisting with price negotiation
    4. Presenting payment options
...
async def chat(request: ChatRequest):
    try:
        ...
        responses = []
        async for chunk in app_workflow.astream(
            {"messages": [("human", request.message)], "history": history},
            stream_mode="values"
        ):
            if chunk["messages"]:
                responses.append(chunk["messages"][-1].content)
        ...
        # Store bot response
        chat_histories[conversation_id].append(
            Message(role="assistant", content=final_response, timestamp=datetime.now().isoformat())
        )

        return ChatResponse(response=final_response, conversation_id=conversation_id, timestamp=datetime.now().isoformat())
